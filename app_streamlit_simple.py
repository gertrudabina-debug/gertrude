...
def créer_zip(fig, x, y, a_crt, b_crt, a_reel, b_reel, sig_noise, m, eqm_history, a_init, b_init, eta, fois):
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    zip_buffer = io.BytesIO()

    amelioration = (1 - eqm_history[-1] / eqm_history[0]) * 100

    txt_contenu = f"""=== PARAMÈTRES DU CALCUL - {timestamp} ===
...
"""
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
        zf.writestr(f'vue_ensemble_{timestamp}.png',  img_buf.read())
        zf.writestr(f'donnees_{timestamp}.csv',       results_df.to_csv(index=False))
        zf.writestr(f'evolution_eqm_{timestamp}.csv', eqm_df.to_csv(index=False))
        zf.writestr(f'parametres_{timestamp}.txt',    txt_contenu)
...
a_reel    = st.sidebar.number_input("a réel [−∞ – +∞]",        min_value=None,   max_value=None, value=0.7,  step=0.01)
b_reel    = st.sidebar.number_input("b réel [−∞ – +∞]",        min_value=None,   max_value=None, value=0.3,  step=0.01)
...
a_init = st.sidebar.number_input("a initial [−∞ – +∞]",        min_value=None,  max_value=None, value=2.0, step=0.01)
b_init = st.sidebar.number_input("b initial [−∞ – +∞]",        min_value=None,  max_value=None, value=2.0, step=0.01)
...
if st.session_state.get("calcul_done"):
    ...
    # Bouton ZIP unique
    zip_buffer, timestamp = créer_zip(
